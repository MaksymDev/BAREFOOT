...
!function($){
	...
	$usof.field['color'] = {
		...
		init: function(options){
			...
			this.$palette = this.$row.find('.colpick_palette');
		...
		colorPalette: function(ev){
			...
			if ($target.hasClass('colpick_palette_add')) {
				$target.addClass('adding');
				palette = {value: this.$input.val()};
				max = this.$palette.children('.colpick_palette_value').length;
				if (max < 8) {
					this.paletteSend(palette);
				}
			}
			if ($target.hasClass('colpick_palette_delete')) {
				currId = $.inArray($target.closest('.colpick_palette_value')[0], this.$palette.find('.colpick_palette_value'));
				if (currId >= 0) {
					colorId = {colorId: currId};
					this.paletteSend(colorId);
					$target.closest('.colpick_palette_value').addClass('deleting');
				}
			}
		},
		setValue: function(value, quiet){
			var r, g, b, a, hexR, hexG, hexB, hashString;
			value = value.trim();
	...
	$usof.field['font'] = {
		...
		init: function(options){
			...
			if (this.curFont != 'none' && this.notLoadedFonts[this.curFont] == undefined) {
		...
		setValue: function(value, quiet){
			...
			if (this.fonts[fontName] === undefined || this.isCurFontUploaded) {
	...
	$usof.field['style_scheme'] = {
		...
		setSchemeButtonStates: function(action){
			if (!this.$schemesContainer) return;
				...
				this.frame.on('open', function(){
					...
					if (this.isMultiple) {
						...
						$.each(value, function(index, file){
							...
							if (fileID) this.frame.state().get('selection').add(wp.media.attachment(fileID));
						}.bind(this));
					} else {
	...
	$usof.field['design_options'] = {
		...
		setValue: function(value){
			...
			this.$input.each(function(index, input){
				...
				if (inputValue !== '' && (regexp = /^position\_[a-z]+\_(.+)$/.exec(name))) checkedPosSwitchers.push(regexp[1]);
			}.bind(this));
	...
	$usof.Group.prototype = {
		...
		setValue: function(value){
			...
			$.each(value, function(index, paramsValues){
				...
				for (var fieldId in groupParams.fields) {
					if (!groupParams.fields.hasOwnProperty(fieldId)) continue;
		...
		groupDel: function($group){
			if (!confirm(this.groupTranslations.deleteConfirm)) return false;
...
!function($){
	...
	$usof.ButtonPreview.prototype = {
		init:function(container){
			...
			this.dependsOn = ['heading_font_family','body_font_family', 'body_fontsize'];
			...
			for (var fieldId in this.groupParams.fields) {
				if (!this.groupParams.fields.hasOwnProperty(fieldId)) continue;
		...
		applyStyle: function() {
			// Font size
			this.$btn.css('font-size', $usof.instance.getValue('body_fontsize')+'px');
			// Font family
			var buttonFont = this.groupParams.getValue('font'),
				fontFamily;
			if ( buttonFont == 'heading' || buttonFont == 'body' ) {
			...
			// Height & Width
			this.$btn.css('padding', this.groupParams.getValue('height')+'em '+this.groupParams.getValue('width')+'em');
			// Corners radius
			this.$btn.css('border-radius', this.groupParams.getValue('border_radius')+'em');
			// Letter spacing
			this.$btn.css('letter-spacing', this.groupParams.getValue('letter_spacing')+'em');
			// Border Width
			this.$btnBefore.css('border-width', this.groupParams.getValue('border_width')+'px');
			// Colors
			...
			this.$btn.css('background-color', colorBg);
			this.$btn.css('border-color', colorBorder);
			this.$btn.css('color', this.groupParams.getValue('color_text'));
			// Hovered colors
			this.$btnAfter.css('background-color', colorBgHover);
			// Shadow
			this.$btn.css('box-shadow', '0 '+this.groupParams.getValue('shadow')/2+'em '+this.groupParams.getValue('shadow')+'em rgba(0,0,0,0.2)');
			// Hover class
			this.$container.usMod('hov', this.groupParams.getValue('hover'));
			// Hovered state
			this.$btn.off('mouseenter mouseleave').on('mouseenter', function() {
				this.$btn.css('box-shadow', '0 '+this.groupParams.getValue('shadow_hover')/2+'em '+this.groupParams.getValue('shadow_hover')+'em rgba(0,0,0,0.2)');
				if (this.groupParams.getValue('hover') == 'fade') {
					this.$btn.css('background-color', colorBgHover);
